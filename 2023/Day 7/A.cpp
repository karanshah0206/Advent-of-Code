...
int main() {
    ...
    while (s != "exit") {
        ...
        else if (handMap.size() == 5) highCard.push_back({ hand, bid });
        else if (handMap.size() == 4) onePair.push_back({ hand, bid });
        else {
            size_t maxCount = 0;
            for (const char& c : s) if (handMap[c] > maxCount) maxCount = handMap[c];
            if (handMap.size() == 3) {
